...
- GTK paint path: `CHELOTYPE_PERF_TRACE=/path/to/perf.tsv` records `input_to_render`, `input_allocs_to_render`, `input_alloc_bytes_to_render`, `gtk_render`, `gtk_paint_interval`, `gtk_paint`, `gtk_paint_rows`, Pango layout cache hit/miss counters, per-render allocations, and `process_rss_kib` from the real app. `gtk_e2e_tracks_held_key_render_and_paint_latency_under_xvfb` holds `a` for 10 seconds under Xvfb, verifies sustained typed input, and gates p95/p99 input-to-render, per-input allocation-to-render, render, paint, per-render allocation, and process-memory behavior.
...
- Nested profile: `xvfb-run -a scripts/profile-240hz.sh --scenario held-key|scroll|scroll-burst|idle|frame-baseline|timer-baseline` runs the real app in a nested X server, drives x11 scenarios with `xdotool`, and summarizes `gtk_frame_interval`, real wall-clock tick cadence, real paint cadence, effective p50 FPS, 240 Hz frame buckets, `gdk_refresh_interval`, `gdk_monitor_refresh_millihz`, GTK tick-work breakdown, paint/render samples, allocation counters, and scroll-frame timing. `scroll` spaces wheel events out; `scroll-burst` sends them back-to-back to verify accumulated-target acceleration and summarizes perf samples from the app-side burst marker onward so shell seeding does not pollute the burst window. `frame-baseline` keeps a nearly empty animation loop active and is the compositor/GDK baseline; `timer-baseline` adds a 240 Hz GLib timeout so main-loop timer cadence can be compared against frame callbacks. Add `--release` for release-mode profiling and `--strict` to fail when p50 frame interval, real paint interval, scroll-frame interval, or paint time exceeds the 4.166 ms 240 Hz budget. Direct xdotool live-desktop profiling is guarded and requires `--allow-live`.
