...
- GTK paint path: `CHELOTYPE_PERF_TRACE=/path/to/perf.tsv` records `input_to_render`, `gtk_render`, and `gtk_paint` durations from the real app. `gtk_e2e_tracks_held_key_render_and_paint_latency_under_xvfb` holds `a` for 10 seconds under Xvfb, verifies sustained typed input, and gates p95/p99 input-to-render, render, and paint latency.
