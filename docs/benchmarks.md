...
- Nested profile: `xvfb-run -a scripts/profile-240hz.sh --scenario held-key|scroll|scroll-burst|scroll-sustain|idle|frame-baseline|timer-baseline|updating-baseline` runs the real app in a nested X server, drives x11 scenarios with `xdotool`, and summarizes `gtk_frame_interval`, real wall-clock tick cadence, real paint cadence, effective p50 FPS, 240 Hz frame buckets, `gdk_refresh_interval`, completed `GdkFrameTimings` presentation cadence/error, `gdk_monitor_refresh_millihz`, GTK tick-work breakdown, paint/render samples, allocation counters, and scroll-frame timing. `scroll` spaces wheel events out; `scroll-burst` sends them back-to-back to verify accumulated-target acceleration and summarizes perf samples from the app-side burst marker onward so shell seeding does not pollute the burst window. `scroll-sustain` repeats the same burst to exercise longer smooth-scroll row-surface cache pressure. `frame-baseline` keeps a nearly empty animation loop active and is the compositor/GDK baseline; `timer-baseline` adds a 240 Hz GLib timeout so main-loop timer cadence can be compared against frame callbacks; `updating-baseline` also calls `gdk_frame_clock_begin_updating()` to test whether continuous low-level GDK frame-clock updates can move cadence closer to 240 Hz. Add `--release`; use `--strict` for end-to-end 240 Hz cadence plus CPU gates, or `--strict-cpu` when nested compositor cadence is expected to miss but CPU work must stay within the 4.166 ms 240 Hz budget. Direct xdotool live-desktop profiling is guarded and requires `--allow-live`.
...
- The GTK smooth-scroll e2e keeps scroll frame and paint p50 under the Xvfb frame-clock budget, and fails if pixel-only scroll frames trigger full terminal renders. `scripts/profile-240hz.sh --strict-cpu` gates p95 CPU work for paint, render, snapshot, tick-work, and smooth-scroll integration; `--strict` additionally gates end-to-end frame, wall, paint, scroll-frame, and completed GDK presentation cadence. Use `scripts/profile-240hz.sh --display-backend native-wayland --scenario scroll-burst --release --strict` for a live-compositor 240 Hz gate without desktop input automation.
- Completed `GdkFrameTimings` samples record actual presentation intervals separately from requested refresh metadata. `gdk_timings_presentation_interval` is the strongest compositor-side cadence signal when available; `gdk_timings_presentation_error` shows how far actual presentation landed from GDK's prediction.
- `scripts/profile-240hz.sh --display-backend weston-headless --scenario frame-baseline --release` is a diagnostic lower bound for the nested backend, not a product 240 Hz proof. On the current machine it reports 240 Hz monitor refresh but about 195 Hz GTK tick and real paint cadence with `gtk_tick_work` around 9 us, so missed 240 Hz cadence there is compositor/GDK pacing rather than terminal render cost. Completed `GdkFrameTimings` presentation intervals confirm the same plateau at about 194 FPS even while timing refresh intervals report 4.166 ms. `timer-baseline` in the same backend shows the GLib timeout can tick around the 240 Hz budget (`glib_timeout_interval` p50 about 4.1 ms) while GDK frame callbacks and real paint intervals remain around 195 Hz, narrowing the nested-backend blocker to GDK/Wayland presentation scheduling rather than main-loop timer precision. `updating-baseline` is the same isolated frame-clock check with `gdk_frame_clock_begin_updating()` enabled; it still stays near the same about-196 Hz nested Weston plateau, so the miss is not caused by missing explicit GDK update requests.
...
scripts/profile-240hz.sh --display-backend weston-headless --scenario scroll-sustain --duration 1 --release --strict-cpu
...
scripts/profile-240hz.sh --display-backend weston-headless --scenario updating-baseline --duration 1 --release
