...
- The GTK smooth-scroll e2e keeps scroll frame and paint p50 under the Xvfb frame-clock budget, and fails if pixel-only scroll frames trigger full terminal renders. `scripts/profile-240hz.sh --strict-cpu` gates p95 CPU work for paint, render, snapshot, tick-work, and smooth-scroll integration; `--strict` additionally gates end-to-end frame, wall, paint, and scroll-frame cadence. Use `scripts/profile-240hz.sh --display-backend native-wayland --scenario scroll-burst --release --strict` for a live-compositor 240 Hz gate without desktop input automation.
