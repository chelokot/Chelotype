...
- Nested profile: `xvfb-run -a scripts/profile-240hz.sh --scenario held-key|scroll|scroll-burst|idle|frame-baseline|timer-baseline` runs the real app in a nested X server, drives x11 scenarios with `xdotool`, and summarizes `gtk_frame_interval`, real wall-clock tick cadence, effective p50 FPS, 240 Hz frame buckets, `gdk_refresh_interval`, `gdk_monitor_refresh_millihz`, GTK tick-work breakdown, paint/render samples, allocation counters, and scroll-frame timing. `scroll` spaces wheel events out; `scroll-burst` sends them back-to-back to verify accumulated-target acceleration. `frame-baseline` keeps a nearly empty animation loop active and is the compositor/GDK baseline; `timer-baseline` adds a 240 Hz GLib timeout so main-loop timer cadence can be compared against frame callbacks. Add `--release` for release-mode profiling and `--strict` to fail when p50 frame interval, scroll-frame interval, or paint time exceeds the 4.166 ms 240 Hz budget. Direct xdotool live-desktop profiling is guarded and requires `--allow-live`.
...
- `scripts/profile-240hz.sh --display-backend weston-headless --scenario frame-baseline --release` is a diagnostic lower bound for the nested backend, not a product 240 Hz proof. On the current machine it reports 240 Hz monitor refresh but about 195 Hz GTK tick cadence with `gtk_tick_work` around 9 us, so missed 240 Hz cadence there is compositor/GDK pacing rather than terminal render cost. `timer-baseline` in the same backend shows the GLib timeout can tick around the 240 Hz budget (`glib_timeout_interval` p50 about 4.1 ms) while GDK frame callbacks remain around 195 Hz, narrowing the nested-backend blocker to GDK/Wayland presentation scheduling rather than main-loop timer precision.
