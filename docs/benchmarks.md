...
- GTK paint path: `CHELOTYPE_PERF_TRACE=/path/to/perf.tsv` records `input_to_render`, `gtk_render`, `gtk_paint`, per-render allocations, and `process_rss_kib` from the real app. `gtk_e2e_tracks_held_key_render_and_paint_latency_under_xvfb` holds `a` for 10 seconds under Xvfb, verifies sustained typed input, and gates p95/p99 input-to-render, render, paint, allocation, and process-memory behavior.
...
- The same GTK held-key e2e keeps per-render allocations bounded and fails if process RSS grows by more than 96 MiB during the 10-second key hold.
