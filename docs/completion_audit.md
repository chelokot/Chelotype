...
- Workspaces, tabs/splits, command blocks, smooth scrolling
  ...
  - Status: partial; visible tabs, split-pane workspace ownership, headless render dumps, GTK split-pane rendering, click-to-activate split routing, split-local click-to-cursor movement, split-local selection, mouse split-resize handles, and basic visible command-block rails exist, but smooth scroll model and rich command-block interactions are not implemented yet.
...
The milestone is not complete. The next highest-value gaps are richer non-Latin IM/preedit layout coverage, smooth scrolling, richer command-block interactions, richer selection/editing semantics, and memory/allocation perf gates.
