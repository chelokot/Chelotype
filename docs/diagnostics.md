...
- Run `scripts/profile-240hz.sh --scenario held-key|scroll|scroll-burst|scroll-sustain|idle|frame-baseline|timer-baseline|updating-baseline` to collect and summarize those perf samples from a nested run. `scroll-burst` sends wheel events without spacing so accumulated target distance must accelerate; `scroll-sustain` repeats that app-side burst to profile longer smooth-scroll cache pressure. `frame-baseline` keeps a minimal animation loop active and isolates compositor/GDK frame-callback cadence from terminal snapshot/render work; `timer-baseline` compares a 240 Hz GLib timeout with GDK frame-callback cadence; `updating-baseline` also enables `gdk_frame_clock_begin_updating()` so explicit low-level frame-clock updating can be compared against ordinary queued redraws. Completed `GdkFrameTimings` samples are summarized as `gdk_timings_*` lines, including actual presentation interval and prediction error. Use `--strict` for an end-to-end 240 Hz cadence gate including completed GDK presentation intervals; use `--strict-cpu` to gate p95 CPU work against the 4.166 ms 240 Hz budget when nested compositor cadence is expected to miss.
