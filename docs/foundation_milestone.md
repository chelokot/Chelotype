...
- Partial: `tests/gtk_e2e_tests.rs` verifies real GTK launch snapshots, ANSI-colored cell export, live GTK Unicode/grapheme/wide-cell JSON export, nonblank real-window screenshots, pixel-level truecolor output, pixel-level narrow cursor shape and position, cursor blink/reset, real Xvfb keyboard input, IM-composed text commit through GTK's input method path, click-to-cursor movement on the current input row, split-pane-local click-to-cursor movement, split-pane-local drag selection, zsh autosuggestion grid/click behavior, terminal mouse-reporting bytes forwarded to the PTY, real mouse drag selection, stable released-selection copy, PRIMARY/CLIPBOARD selection export, visible split-pane rendering, click-to-activate routing between split panes, split-divider drag resize, real GTK held-key render/paint latency, and real GTK window resize without GTK warnings.
...
- Weak: first-class keyboard and mouse support has mode-aware click press/release/drag plumbing, terminal mouse-reporting e2e, basic shell cursor placement on the current input row in both headless and GTK paths, headless soft-wrapped input cursor placement, semantic prompt continuation row cursor placement in unit coverage, GTK IM commit support for composed text, a local grid selection model whose drag state stops on release, real GTK drag-selection e2e, and PRIMARY/CLIPBOARD export; richer multi-command cursor placement and preedit visualization are still primitive.
...
1. Add IM preedit visualization and stronger non-Latin layout coverage.
