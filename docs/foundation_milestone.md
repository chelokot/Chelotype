...
- Weak: first-class keyboard and mouse support has mode-aware click press/release/drag plumbing, terminal mouse-reporting e2e, basic shell cursor placement on the current input row in both headless and GTK paths, headless soft-wrapped input cursor placement, semantic prompt continuation row cursor placement in unit coverage, GTK IM preedit visualization plus committed composed text, a local grid selection model whose drag state stops on release, real GTK drag-selection e2e, and PRIMARY/CLIPBOARD export; richer multi-command cursor placement and complex IM/preedit styling scenarios are still primitive.
- Weak: screenshots now prove the GTK surface is nonblank, truecolor text reaches pixels, and the cursor is rendered as a blinking narrow caret that resets visible after input; render-state snapshots now export transient IM preedit overlays at the terminal cursor before commit; GTK held-key e2e now gates visible render/paint p95/p99 plus per-frame allocations and RSS growth, but allocation attribution per keystroke is still missing.

Next implementation order

1. Add stronger non-Latin layout and complex IM/preedit styling coverage.
2. Add smooth scrolling and richer command-block interactions.
