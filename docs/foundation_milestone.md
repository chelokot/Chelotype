...
- Weak: first-class mouse support has mode-aware click press/release/drag plumbing, terminal mouse-reporting e2e, basic shell cursor placement on the current input row, a local grid selection model whose drag state stops on release, real GTK drag-selection e2e, and PRIMARY/CLIPBOARD export; multiline/prompt-aware shell cursor placement is still primitive.
